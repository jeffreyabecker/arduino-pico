...
namespace littlefs_impl
{
    ...
    int LittleFSImpl::lfs_lock(const struct lfs_config *c)
    {

        LittleFSImpl *me = reinterpret_cast<LittleFSImpl *>(c->context);
#ifdef __FREERTOS
        if (xSemaphoreTake(me->_mutex, portMAX_DELAY) == pdTRUE)
        {
            return 0;
        }
        else
...
#else
        mutex_enter_blocking(&me->_mutex);
        return 0;
#endif
    }
    int LittleFSImpl::lfs_unlock(const struct lfs_config *c)
    {

        LittleFSImpl *me = reinterpret_cast<LittleFSImpl *>(c->context);
#ifdef __FREERTOS
        if (xSemaphoreGive(me->_mutex) == pdTRUE)
        {
            return 0;
        }
        else
